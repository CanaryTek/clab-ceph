...
task :init_vms do |t|
  ...
  %x[sudo brctl addbr osdbr0]
  %x[sudo ip link set osdbr0 up]
	 @vms.each do |vm|
    name=vm
    ...
    %x[sudo mkdir -p #{vm_dir}]
    create_user_data(name,ssh_key)
    create_meta_data(name)
    %x[cd #{vm_dir} ; sudo mkisofs -output cidata.iso -volid cidata -joliet -r meta-data user-data]
    %x[sudo cp images/#{@config["images"]["default"]["name"]} #{vm_dir}/#{vm}-disk1.qcow2]
    %x[sudo qemu-img resize #{vm_dir}/#{vm}-disk1.qcow2 #{@config["vms"][vm]["disks"]["disk1"]["size"]}G]
...
task :init_vm_sub do |t|
	%x[sudo btrfs subvolume create vm]
end

desc "Create snapshot"
task :snap_create, [:name] do |t,args|
	%x[sudo test -d .snapshots || mkdir -p .snapshots]
	%x[sudo btrfs subvolume snapshot -r vm .snapshots/#{args[:name]}]
end

desc "List snapshot"
task :snap_list do |t,args|
	system("sudo btrfs subvolume list -s .")
end
